...
import { useCreateMenuItemMutation, useGetMenuItemByIdQuery,useUpdateMenuItemMutation } from '../../apis';
...
const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (file) {
    ...
    const isImageTypeValid = validImgTypes.filter((img) => {
      return img === imgType;
    });
      ...
      <div className="row mt-3">
        <div className="col-md-7 ">
          ...
          <select
            ...
            onChange={handleUserInput}
          >
            {Categories.map(it=>(<option value={it}>{it}</option>))}
          </select>
          <input
            type="number"
            className="form-control mt-3"
